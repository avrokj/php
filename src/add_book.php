...
if (isset($_POST['add_book'])) {
  ...
  $stmt = $pdo->prepare('INSERT INTO books (title, language, price, pages, summary, stock_saldo, type) VALUES (:title, :language, :price, :pages, :summary, :stock_saldo, :type) WHERE id = :id');
      ...
      <form action="./add_book.php" method="post" id="add_book">
        <div class="grid grid-cols-4 gap-4">
          ...
          <div class="col-span-3">
            ...
            <input type='text' name='language' value='' class='dark:bg-slate-500 border dark:border-cyan-600 shadow rounded h-10 shadow p-4 focus:shadow-outline'>
            <label class="block font-bold mt-2">Hind</label>
            <input type='number' name='price' step='any' value='' class='dark:bg-slate-500 border dark:border-cyan-600 shadow rounded h-10 shadow p-4 focus:shadow-outline'> € <small><em>(keskmine lehe hind: <?= round($book['price'] / $book['pages'], 2); ?> €)</em></small></p>

            <div class="flex flex-wrap -mx-3 mb-2">
              <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                <label class="block font-bold mt-2">Lehti</label>
                <input type='number' name='pages' value='' class='w-full dark:bg-slate-500 border dark:border-cyan-600 shadow rounded h-10 shadow p-4 focus:shadow-outline'>
              </div>
              <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0"><label class="block font-bold mt-2">Laoseis</label>
                <input type='number' min="0" name='stock_saldo' value='' class='w-full dark:bg-slate-500 border dark:border-cyan-600 shadow rounded h-10 shadow p-4 focus:shadow-outline'>
              </div>
